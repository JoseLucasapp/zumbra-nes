...
output_dir="${Z22_PACKAGE_DIR:-dist}"
arch="${Z22_ARCH:-amd64}"
...
grep -q '^Z22 desktop session complete$' build/desktop-headless.txt
...
appdir="$output_dir/zumbra-nes-0.4.0-linux-${arch}.AppDir"
deb="$output_dir/zumbra-nes_0.4.0_${arch}.deb"
test -x "$appdir/AppRun"
test -f "$deb"
ZUMBRA_DESKTOP_HEADLESS=1 "$appdir/AppRun" | tee build/appdir-headless.txt
grep -q '^Z22 desktop session complete$' build/appdir-headless.txt
dpkg-deb --info "$deb" > build/deb-info.txt
dpkg-deb --contents "$deb" > build/deb-contents.txt
grep -q ' Version: 0.4.0' build/deb-info.txt
...
(
    cd "$output_dir"
    find . -maxdepth 1 -type f ! -name 'SHA256SUMS-Z22.txt' -print0 | sort -z | xargs -0 sha256sum > SHA256SUMS-Z22.txt
)

echo "Z22 Linux packages created in $output_dir."
