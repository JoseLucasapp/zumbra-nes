## 0.5.45 desktop/input/homebrew check
...
## 0.5.45 desktop intro/input check
...
## 0.5.45 desktop app-build check

A 0.5.45 exige que `zumbra app build --manifest zumbra-app.toml --target linux --arch amd64 --release -o build/zumbra-nes` gere `build/zumbra-nes` sem `types: function has conflicting return types: null and bool`.
